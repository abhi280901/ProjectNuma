...
program = "SplitFed_MLP_CTGAN_ep10_lep_1_lr1e-3_top2_2"
print(f"---------{program}----------")
...
hidden_dim = 32      # Hidden representation dimension
...
# =============================================================================
# 2. Define the Client-Side MLP Model
...
# =============================================================================
# 3. Define the Server-Side MLP Model
...
# =============================================================================
net_glob_client = MLPClient(input_dim, hidden_dim)
...
net_glob_server = MLPServer(hidden_dim, output_dim)
if torch.cuda.device_count() > 1:
...
# =============================================================================
df_full = pd.read_parquet('../BIS_data/Final_BIS_Data_top2_schema.parquet')
...
Xy_train_ctgan = pd.read_csv("BIS_CTGAN_Full_Top2.csv")
...
for iter in range(epochs):
    ...
    if len(loss_test_collect) > 0:
        ...
        if current_loss < best_test_loss:
            ...
            torch.save(net_glob_client.state_dict(), "best_client_model_CTGAN_MLP_V2_wo_drop_2.pth")
            torch.save(net_glob_server.state_dict(), "best_server_model_CTGAN_MLP_V2_wo_drop_2.pth")
