...
program = "SplitFed_ResNet_Smote_ep10_lep_2_lr1e-3"
print(f"---------{program}----------")
...
df = pd.read_parquet('../BIS_data/Final_BIS_Data.parquet')
df = df.reset_index(drop=True)
print("Dataset shape:", df.shape)

# Separate features and target (assumes column "label" holds the binary target)
X = df.drop('laundering_schema_type', axis=1).values
y = df['laundering_schema_type'].values
...
dataset_train = FraudDataset(X_train_smote, y_train_smote)
...
for iter in range(epochs):
    ...
    if len(loss_test_collect) > 0:
        ...
        if current_loss < best_test_loss:
            ...
            torch.save(net_glob_client.state_dict(), "best_client_model_Smote_ResNet.pth")
            torch.save(net_glob_server.state_dict(), "best_server_model_Smote_ResNet.pth")
