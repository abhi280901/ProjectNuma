...
program = "SplitFed_FraudResNet_Fraud_Undersampling_ep10_lep_2_lr1e-3"
print(f"---------{program}----------")
...
# =============================================================================
net_glob_client = FraudResNetClient(input_dim, hidden_dim, num_blocks=num_blocks)
...
class Client(object):
    def __init__(self, net_client_model, idx, lr, device, dataset_train=None, dataset_test=None, idxs=None, idxs_test=None):
        ...
        self.device = device
        self.lr = lr
        self.local_ep = 2   # Number of local epochs per round
...
# Separate features and target (assumes column "Class" holds the binary target)
...
X_train_undersampled = train_df_undersampled.drop('laundering_schema_type', axis=1).values
y_train_undersampled = train_df_undersampled['laundering_schema_type'].values

print("After Undersampling, training set shape:", X_train_undersampled.shape)

X_test = X_test.values
y_test = y_test.values

# Update input_dim based on data (should remain the same as before)
input_dim = X_train_undersampled.shape[1]

dataset_train = FraudDataset(X_train_undersampled, y_train_undersampled)
...
for iter in range(epochs):
    ...
    if len(loss_test_collect) > 0:
        ...
        if current_loss < best_test_loss:
            ...
            torch.save(net_glob_client.state_dict(), "best_client_model_undersampling_ResNet.pth")
            torch.save(net_glob_server.state_dict(), "best_server_model_undersampling_ResNet.pth")
